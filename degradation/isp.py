...
from degradation.Demosaicing_malvar2004 import demosaicing_CFA_Bayer_Malvar2004
import random
from PIL import Image


class ISP:
    def __init__(self, curve_path="degradation"):
    ...
    def XYZ2CAM(self, img, M_xyz2cam=0):
        if type(M_xyz2cam) is int:
            ...
            M_xyz2cam = (
                self.xyz2cam_all[0, :] * cam_index[0, 0]
    ...
    def cbdnet_noise_generate_srgb(self, img):
        ...
        wb_mask = self.WB_Mask(
            img_mosaic_noise, pattern, 1 / self.fr_now, 1 / self.fb_now
        )
        ...
        # Step 1 : tone mapping
        img_Irgb = self.CRF_Map(img_IL, index=icrf_index) * 255.0
        img_Irgb_gt = self.CRF_Map(img_IL_gt, index=icrf_index) * 255.0

        return img_Irgb_gt.astype(np.uint8), img_Irgb.astype(np.uint8)
...
if __name__ == "__main__":
    ...
    gt = Image.fromarray(noise)
    gt.save("butterfly_result.png")
